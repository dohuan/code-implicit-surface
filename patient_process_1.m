function out = patient_process_1(pat_info,option)
...
%[S_test_est_down,~] = bin_search(est_train,est_test_down,thres_train_min,S_temp,option,0);

S_test_est_up = [];
for j=1:size(est_test_up,1)
    if (est_test_up(j,1)>=0&&est_test_up(j,1)<=thres_test_min)
        S_test_est_up = [S_test_est_up;S_temp(j,:)];
    end
end

S_test_est_down = [];
for j=1:size(est_test_down,1)
    if (est_test_down(j,1)>=0&&est_test_down(j,1)<=thres_test_min)
...
out.S_est_up = surface_refiner(S_test_est_up);
out.S_est_down = surface_refiner(S_test_est_down);
out.S_true = S_true;
